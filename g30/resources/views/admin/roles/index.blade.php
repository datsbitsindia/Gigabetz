        ...
        { data: "rol_id", render: function ( data, type, full, meta ) 
            {
                ...
                var _link='';
                @if(strpos(' '.$data,'E')>0) 			  
                    _link+= '&nbsp;⁞&nbsp;<span class="span-link sub_menu_tab_a" parent-data-id="{{$mod_action->parent_data_id}}" parent-title="{{$mod_action->parent_title}}" data-action="E" data-key="'+buttonID+'" data-link="roles/'+buttonID+'/edit">Edit</span>'  
                @endif 
                @if(strpos(' '.$data,'D')>0) 			  
